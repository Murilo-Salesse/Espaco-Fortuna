...
interface Configuracao {
  ...
  banheiros: number; vagas: number; comodidades: string[]
  fotos: string[]
}
...
const IconUpload  = () => <svg width="18" height="18" fill="none" stroke="#a8a29e" strokeWidth="1.5" viewBox="0 0 24 24"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
...
export default function AdminPage() {
  ...
  async function salvarCalendario() {
    ...
    if (bloquear.length) {
      await fetch('/api/datas/bloquear', {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
        body:    JSON.stringify({ datas: bloquear, acao: 'bloquear' }),
      })
    }
    if (liberar.length) {
      await fetch('/api/datas/bloquear', {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
        body:    JSON.stringify({ datas: liberar, acao: 'liberar' }),
      })
    }
  ...
  async function confirmarReserva(reserva: Reserva) {
    ...
    if (res.ok) {
      ...
      carregarDatas()
      showToast('Reserva confirmada! Datas bloqueadas.')
    } else {
  ...
  async function cancelarReserva(token: string) {
    ...
    if (res.ok) {
      setReservas(prev => prev.map(r => r.token === token ? { ...r, status: 'cancelada' } : r))
  ...
  async function salvarDetalhes() {
    await fetch('/api/configuracao', {
      method:  'PUT',
      headers: { 'Content-Type': 'application/json' },
      body:    JSON.stringify({ ...config, comodidades }),
    })
    ...
    <div className="font-sans bg-stone-50 text-stone-800 min-h-screen">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-white border-b border-stone-200 h-14 flex items-center px-4 justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={logout} className="text-xs text-stone-400 hover:text-red-500 transition-colors hidden sm:inline">
            Sair
          </button>
        </div>
      </header>
      ...
      <div className="flex pt-14 min-h-screen">
        ...
        >
          <nav className="flex-1 px-3 space-y-0.5" style={{ width: '200px' }}>
            {([
              ...
              { id: 'fotos',     label: 'Fotos',     icon: <IconPhoto /> },
        ...
        >
          ...
          {tab === 'dashboard' && (
            <div className="animate-in">
              ...
              <div className="bg-white border border-stone-200 rounded-2xl overflow-hidden">
                ...
                {reservas.filter(r => r.status !== 'cancelada').slice(0,4).map(r => (
                  <div key={r.id} className="px-4 md:px-6 py-4 flex items-center justify-between border-b border-stone-100 last:border-0">
                    ...
                    <div className="flex items-center gap-2">
                      <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${r.status === 'confirmada' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>
                        {r.status}
                      </span>
                      {r.status === 'pendente' && (
                        <button onClick={() => setConfirmModal(r)} className="text-xs border border-green-300 text-green-700 px-3 py-1 rounded-full hover:bg-green-50 transition-colors hidden sm:inline">
                          Confirmar
                        </button>
                      )}
                    </div>
                ...
                {reservas.length === 0 && (
                  <p className="px-6 py-8 text-center text-sm text-stone-400">Nenhuma reserva ainda.</p>
                )}
              </div>
            </div>
          )}
                ...
                {reservas.map(r => (
                  <div key={r.id} className={`bg-white border rounded-2xl p-4 ${r.status === 'pendente' ? 'border-amber-200' : 'border-stone-200'}`}>
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <span className={`text-xs font-medium px-2.5 py-1 rounded-full flex-none ${r.status === 'confirmada' ? 'bg-green-100 text-green-700' : r.status === 'pendente' ? 'bg-amber-100 text-amber-700' : 'bg-stone-100 text-stone-400'}`}>
                        {r.status}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-stone-100">
                      ...
                      <div className="flex gap-2">
                        {r.status === 'pendente' && (
                          <button onClick={() => setConfirmModal(r)} className="text-xs border border-green-300 text-green-700 px-3 py-1.5 rounded-full hover:bg-green-50 transition-colors">Confirmar</button>
                        )}
                        {r.status !== 'cancelada' && (
                          <button onClick={() => cancelarReserva(r.token)} className="text-xs text-stone-400 hover:text-red-500 transition-colors">Cancelar</button>
                        )}
                      </div>
                    </div>
                  </div>
                    ...
                    {reservas.map(r => (
                      <tr key={r.id}>
                        <td className="px-6 py-4">
                          ...
                          <div className="text-xs text-stone-400">{r.email} · {r.telefone}</div>
                        </td>
                        ...
                        <td className="px-4 py-4">
                          <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${r.status === 'confirmada' ? 'bg-green-100 text-green-700' : r.status === 'pendente' ? 'bg-amber-100 text-amber-700' : 'bg-stone-100 text-stone-400'}`}>
                            {r.status}
                          </span>
                        </td>
          ...
          {tab === 'calendario' && (
            <div className="animate-in">
              <div className="mb-6">
                ...
                <p className="text-stone-400 text-sm mt-1">Clique em dias para bloquear ou liberar. Depois clique em Salvar.</p>
              </div>
              <div className="bg-white border border-stone-200 rounded-2xl p-4 md:p-6 max-w-md">
                ...
                <div className="grid grid-cols-7 gap-1">
                  {diasCalMes.map((date, i) => {
                    ...
                    return (
                      <div key={date.toISOString()} className={getDiaCls(date)} onClick={() => toggleDia(date)}>
                        {date.getDate()}
                      </div>
                    )
                  })}
          ...
          {tab === 'fotos' && (
            <div className="animate-in">
              <div className="mb-6"><h1 className="text-xl md:text-2xl font-serif text-stone-900">Fotos</h1><p className="text-stone-400 text-sm mt-1">Gerencie as fotos exibidas no site.</p></div>
              <div className="bg-white border border-stone-200 rounded-2xl p-4 md:p-6">
                <label className="drag-zone rounded-xl p-6 md:p-8 text-center cursor-pointer mb-6 block">
                  <input type="file" accept="image/*" multiple className="hidden" />
                  <div className="w-10 h-10 rounded-full bg-stone-100 flex items-center justify-center mx-auto mb-3"><IconUpload /></div>
                  <p className="text-sm text-stone-400 mb-1">Arraste as fotos ou <span className="text-green-600">clique para selecionar</span></p>
                  <p className="text-xs text-stone-300">JPG, PNG · Máx 5MB</p>
                </label>
                <p className="text-xs text-stone-400 font-medium uppercase tracking-wide mb-3">Fotos cadastradas</p>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                  {(config?.fotos ?? []).length === 0 ? (
                    ['Área externa','Piscina','Churrasqueira'].map((label, i) => (
                      <div key={label} className="group relative aspect-video rounded-xl overflow-hidden flex items-end p-2"
                        style={{ background: ['linear-gradient(135deg,#064e3b,#1D9E75)','linear-gradient(135deg,#1e3a5f,#3b82f6)','linear-gradient(135deg,#7c3f00,#d97706)'][i] }}>
                  ...
                  <div className="aspect-video border-2 border-dashed border-stone-200 rounded-xl flex items-center justify-center cursor-pointer hover:border-green-400 transition-colors">
        ...
        <div className="modal-overlay open" onClick={e => { if (e.target === e.currentTarget) setConfirmModal(null) }}>
          <div className="bg-white rounded-2xl border border-stone-200 p-6 w-full max-w-sm shadow-xl">
            ...
            <div className="bg-stone-50 rounded-xl p-4 mb-5 space-y-2">
              {[['Cliente', confirmModal.nome],['Data início', fmt(confirmModal.data_inicio)],['Data fim', fmt(confirmModal.data_fim)],['Valor', `R$ ${Number(confirmModal.valor_total).toLocaleString('pt-BR')}`]].map(([l,v]) => (
        ...
        <div className="modal-overlay open" onClick={e => { if (e.target === e.currentTarget) setComodModal(false) }}>
          <div className="bg-white rounded-2xl border border-stone-200 p-6 w-full max-w-sm shadow-xl">
            ...
            <input
              type="text"
              value={novaComod}
              onChange={e => setNovaComod(e.target.value)}
              onKeyDown={e => { if (e.key === 'Enter') { if(novaComod.trim()) { setComodidades(p => [...p, novaComod.trim()]); setNovaComod(''); setComodModal(false) } } }}
              placeholder="Ex: Sauna, Área de jogos..."
              autoFocus
      ...
      {}
      {toast && (
        <div className="fixed bottom-20 md:bottom-6 right-4 md:right-6 bg-stone-900 text-white text-sm px-5 py-3 rounded-full shadow-lg z-50 fade-up">
          {toast}
        </div>
      )}
    </div>
